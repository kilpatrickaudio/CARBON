...
#define STEP_EDIT_ADJUST_PROBABILITY 4
...
void step_edit_adjust_probability(int change, int shift) {
    sedits.edit_timeout = panel_menu_get_timeout();
    if(sedits.event_pos == STEP_EDIT_EVENT_POS_ALL) {
        step_edit_adjust_step(change << 1, STEP_EDIT_ADJUST_PROBABILITY, 0, shift);
    }
    else {
        step_edit_adjust_step(change << 1, STEP_EDIT_ADJUST_PROBABILITY, 1, shift);
    }
}
        ...
        for(i = start; i < (start + num); i ++) {
            ...
            switch(mode) {
                ...
                case STEP_EDIT_ADJUST_START_DELAY:
                    ...
                    inhibit_preview = 1;  // user must retrigger edit to hear
                    break;
	        case STEP_EDIT_ADJUST_PROBABILITY:
		  event.probability = seq_utils_clamp(event.probability + change,
		      STEP_EDIT_MIN_PROBABILITY, STEP_EDIT_MAX_PROBABILITY);
		  inhibit_preview = 1;
		  sedits.last_adjust_type = STEP_EDIT_ADJUST_PROBABILITY;
		  song_set_step_event(sedits.scene, sedits.track,
		      sedits.step_pos, i, &event);
		  break;
            }
...
void step_edit_update_display(void) {
    ...
    for(i = 0; i < SEQ_TRACK_POLY; i ++) {
        ...
        switch(event.type) {
		    ...
		    case STEP_EDIT_ADJUST_PROBABILITY:
		        // display probability
		        sprintf(tempstr, "%-3d", event.probability);
			gui_set_status_text_part(3, xpos, 4, tempstr);
			break;
                }
                break;
